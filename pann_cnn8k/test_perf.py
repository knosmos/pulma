...
model = Cnn14_8k(sample_rate=8000, window_size=256, hop_size=80, mel_bins=64, fmin=50, fmax=4000, classes_num=527)
...
with torch.no_grad():
    ...
    full_labels = full_labels.reshape(
        (full_labels.shape[0] * full_labels.shape[1], 6)
    ).transpose(1, 0).cpu().numpy()
    full_outputs = full_outputs.reshape(
        (full_outputs.shape[0] * full_outputs.shape[1], 6)
    ).transpose(1, 0).cpu().numpy()
    ...
    for i in range(6):
        ...
        plt.plot(recall, precision, label="IEDRWS"[i])
    ...
    plt.legend()
    plt.savefig(f"pr_{exp_name}.svg")