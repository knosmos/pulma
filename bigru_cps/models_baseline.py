...
class Cnn14_8k_deconv(nn.Module):
    ...
    def forward(self, x, mixup_lambda=None):
        ...
        #segmentwise_output = torch.sigmoid(self.fc_audioset(x))
        segmentwise_nosigmoid = self.fc_audioset(x)
        segmentwise_output = torch.sigmoid(segmentwise_nosigmoid)
        #(clipwise_output, _) = torch.max(segmentwise_output, dim=1)
        ...
        framewise_nosigmoid = self.interpolator(segmentwise_nosigmoid)
        framewise_nosigmoid = pad_framewise_output(framewise_nosigmoid, frames_num)

        #output_dict = {'framewise_output': framewise_output, 
        #    'clipwise_output': clipwise_output}

        return (framewise_output, framewise_nosigmoid)
