...
# Custom imports
from H5_dataset_single import HF_Lung_Dataset
from models_deconv import TwinNetworkGRUCAS
import people_split
#import librosa
...
model = TwinNetworkGRUCAS(64, 4, device, single=(not args.cps), gru_layers=2)
...
for epoch in range(args.epochs):
    ...
    with torch.no_grad():
        ...
        full_labels = full_labels.reshape(
            full_labels.shape[0] * full_labels.shape[1]
        ).cpu().numpy()
        full_outputs = full_outputs.reshape(
            full_outputs.shape[0] * full_outputs.shape[1]
        ).cpu().numpy()

        val_scores = sklearn.metrics.f1_score(
            full_labels,
            full_outputs > threshold,
            zero_division=0
        )
        ...
        if score_sum > best_val_f1:
            ...
            torch.save(model.state_dict(), f"model_{args.name}_f1.pt")
