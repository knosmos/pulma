...
class BiGRUNetwork(nn.Module):
    def __init__(self, input_size, num_classes, device, gru_layers=3):
        ...
        self.backbone = Cnn14_8k_deconv(sample_rate=8000, window_size=256, hop_size=80, mel_bins=64, fmin=50, fmax=4000, classes_num=527)
        self.gru = nn.GRU(256, self.hidden_size, self.num_layers, bidirectional=True, batch_first=True, dropout=0.2)
    ...
    def forward(self, x):
        ...
        out, _ = self.gru(x, h0)
        #print("gru out shape:",out.shape)
        # out: tensor of shape (batch_size, seq_length, hidden_size)

        segmentwise_output = torch.sigmoid(self.fc(out))
        #print("fc2 out shape:",segmentwise_output.shape)
        #(clipwise_output, _) = torch.max(segmentwise_output, dim=1)

        # Get framewise output
        framewise_output = self.interpolator(segmentwise_output)
        #print("interpolated:",framewise_output.shape)
        framewise_output = pad_framewise_output(framewise_output, frames)
        #print("padded:",framewise_output.shape)
        #framewise_output = nn.functional.interpolate(segmentwise_output, size=frames, mode='nearest', align_corners=True)

        return framewise_output

class BiGRUNetworkCAS(nn.Module):
    def __init__(self, input_size, num_classes, device, gru_layers=3):
        super(BiGRUNetworkCAS, self).__init__()
        self.hidden_size = 1024
        self.num_layers = gru_layers
        self.backbone = Cnn14_8k_deconv(sample_rate=8000, window_size=256, hop_size=80, mel_bins=64, fmin=50, fmax=4000, classes_num=527)
        self.gru = nn.GRU(256, self.hidden_size, self.num_layers, bidirectional=True, batch_first=True, dropout=0.2)
    ...
    def forward(self, x):
        ...
        framewise_output = self.interpolator(segmentwise_output)
        #print("interpolated:",framewise_output.shape)
        framewise_output = torch.squeeze(pad_framewise_output(framewise_output, frames))
        #print("padded:",framewise_output.shape)
        #framewise_output = nn.functional.interpolate(segmentwise_output, size=frames, mode='nearest', align_corners=True)
...
class TwinNetworkGRUCAS(nn.Module):
    def __init__(self, input_size, num_classes, device, single=False, gru_layers=3):
        super(TwinNetworkGRUCAS, self).__init__()
        self.l = BiGRUNetworkCAS(input_size, num_classes, device, gru_layers=gru_layers)
        self.r = BiGRUNetworkCAS(input_size, num_classes, device, gru_layers=gru_layers)
        self.device = device
        self.single = single
        self.init_weights()
