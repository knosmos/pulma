...
def load_sample(filename):
    ...
    # extract spec
    spec = librosa.feature.melspectrogram(y=audio, sr=8000, n_mels=64, n_fft=256, hop_length=80)
...
def load_sample_combined(filename):
    # load audio
    audio = librosa.load(filename, sr=4000)[0]
        ...
        for label in labels:
            if label[0] <= i * 80 / 8000 <= label[1]:
                if label[2] == "I":
                    ...
                    y[i, 2] = 1
                elif label[2] in ["Rhonchi", "Wheeze", "Stridor"]:
                    y[i, 3] = 1
