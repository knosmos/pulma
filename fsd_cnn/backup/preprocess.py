...
def filter_audio(audio, denoise=True):
    ...
    # normalize to third quartile
    filtered = filtered / np.nanpercentile(np.abs(filtered), 75)

    # get rid of nan if it exists
...
if __name__ == "__main__":
    ...
    for file in tqdm(os.listdir(folder)):
        ...
        t2 = np.linspace(0, len(filtered) / 32000, num=len(filtered))
        ...
        librosa.display.specshow(
            librosa.power_to_db(
                librosa.feature.melspectrogram(y=filtered, sr=32000, n_mels=64, n_fft=1024, hop_length=320),
                ref=np.max,
            ),
            x_axis='time',
            y_axis='mel',
