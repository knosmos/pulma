...
class ConvBlock(nn.Module):
    ...
    def forward(self, input, use_pool=True, pool_size=(2, 2), pool_type='avg'):
...
class Cnn14_DecisionLevelMax(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        amin = 1e-10
        top_db = None
        self.interpolate_ratio = 8     # Downsampled ratio
    ...
    def forward(self, x, mixup_lambda=None):
        ...
        #print("ConvBlock 1:", x.shape)

        #x = self.conv_block2(x, pool_size=(2, 2), pool_type='avg')
        x = self.conv_block2(x, use_pool=False)
        x = F.dropout(x, p=0.2, training=self.training)

        #print("ConvBlock 2:", x.shape)

        x = self.conv_block3(x, pool_size=(2, 2), pool_type='avg')
        x = F.dropout(x, p=0.2, training=self.training)

        #print("ConvBlock 3:", x.shape)

        #x = self.conv_block4(x, pool_size=(2, 2), pool_type='avg')
        x = self.conv_block4(x, use_pool=False)
        x = F.dropout(x, p=0.2, training=self.training)

        #print("ConvBlock 4:", x.shape)

        x = self.conv_block5(x, pool_size=(2, 2), pool_type='avg')
        x = F.dropout(x, p=0.2, training=self.training)
        
        #print("ConvBlock 5:", x.shape)

        #x = self.conv_block6(x, pool_size=(2, 2), pool_type='avg')
        x = self.conv_block6(x, use_pool=False)
        x = F.dropout(x, p=0.2, training=self.training)

        #print("ConvBlock 6:", x.shape)
        ...
        #print("Mean and Pooling:", x.shape)
